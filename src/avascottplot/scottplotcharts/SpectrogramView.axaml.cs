...
namespace scottplotcharts
{
    public partial class SpectrogramView : UserControl
    {
        ...
        private List<SpectrumData> _dataList = new List<SpectrumData>();
        ...
        {
            if (_dataList.Count > _displayDataLength)
            {
            ...
            while (true)
            {
                try
                {
                    if (_drawEvent.WaitOne())
                    {
                        ...
                        var dataArr = _dataList.ToArray();
                        int yLen = dataArr.Length;
                        int xLen = dataArr[0].Data.Length;
                        double[,] buff = new double[yLen, xLen];

                        for (int y = 0; y < yLen; y++)
                        {
                            for (int x = 0; x < xLen; x++)
                            {
                                buff[y, x] = dataArr[y].Data[x];
                            }
                        }

                        _heatMap.Update(buff);
        ...
        private void CreatePlot()
        {
            ...
            _mPlot.Plot.SetAxisLimitsY(0, _displayDataLength);
            _mPlot.Plot.SetAxisLimitsX(0, 4096);
            _mPlot.Plot.SetOuterViewLimits(0, 4096, 0, _displayDataLength);

            double?[,] data = new double?[500, 4096];
            _heatMap = _mPlot.Plot.AddHeatmap(data, Colormap.Rain, true);
                ...
                try
                {
                    if (_drawEvent.WaitOne())
                    {
                        ...
                        { // 当超过2条数据时就处理
                            ...
                            _mPlot.Plot.RenderLock();
                            var addHm = _mPlot.Plot.AddHeatmap(intensities,   Colormap.Rain, true);
                            _mPlot.Plot.RenderUnlock();
                            ...
                            //_mPlot.Plot.RenderLock();
                            //var addHm = _mPlot.Plot.AddHeatmap(intensities,  Colormap.Rain, false);
                            //_mPlot.Plot.RenderUnlock();
